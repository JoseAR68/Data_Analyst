...
-- La nueva tabla debe ser capaz de identificar cada carta de una manera única y establecer una relación apropiada con las otras 
-- dos tablas (“transaction” y “company”). Después de crear la tabla, será necesario introducir la información del documento 
-- llamado "dades_introducir_credit". Recuerde mostrar el diagrama y hacer una breve descripción de él.
...
-- COMPROBACION DE LA CREACION DE LA TABLA credit_card
SHOW TABLES
;

-- CREACION DE UNA FOREIGN KEY EN LA TABLA TRANSACTION 
-- CON REFERENCIA A LA TABLA credit_ card.

-- deshabilitacion de la comprobacion de las restriciones de la foreign key.
SET FOREIGN_KEY_CHECKS = 0
;
-- creacion de la foreign key-
ALTER TABLE transaction ADD foreign key (credit_card_id) REFERENCES credit_card(id)
;
-- habilitacion de la comprobacion de las restriciones de la foreign key.
SET FOREIGN_KEY_CHECKS = 1
;

-- COMPROBACION DE LOS INDICES CREADOS.
SHOW INDEXES FROM transaction
;
SHOW INDEXES FROM company
;
SHOW INDEXES FROM credit_card
;

-- INTRODUCCION DE DATOS EN LA TABLA credit_card.
...
-- La información que se mostrará para este registro es: R323456313122135717699999. Recuerde demostrar que el cambio se hizo.

-- comprobacion de los datos actuales del usuario CcU-2938.
SELECT * 
FROM credit_card
WHERE id = 'CcU-2938'
;

-- Modificación del Iban del cliente CcU-2938
UPDATE credit_card
SET iban = 'R323456313122135717699999'
WHERE id = 'CcU-2938'
;

-- Comprobacion del registro actualizado.
SELECT *
FROM credit_card
WHERE id = 'CcU-2938'
;
...
-- NIVEL 1
-- EJERCICIO 3
-- En la tabla de "transacción", un nuevo usuario entra con nueva informacion.

-- Comprobación del registro con id 108B1D1D-5B23-A76C-55EF-C568E49A99DD.
-- Se comprueba que no existe ningun registro con este id.
SELECT *
FROM transaction
WHERE id = '108B1D1D-5B23-A76C-55EF-C568E49A99DD'
;

-- Se comprueba la existencia del nuevo valor en transaction.company_id en el campo company.id
-- Al ser transaction.company_id la foreign key de la primary key company.id el nuevo valor para transaction.company_id debe
-- existir en company.id antes de poder ser creado en transaction.company_id.
SELECT *
FROM company
WHERE id = 'b-9999'
;

-- Añadir el valor 'b-9999' en el campo id de la tabla company.
INSERT INTO company(id) 
VALUES ('b-9999')
;

-- Se comprueba la existencia del nuevo valor en transaction.credit_card_id en el campo credit_card.id
-- Al ser transaction.credit_card_id la foreign key de la primary key credit_card.id el nuevo valor para transaction.credit_card_id debe
-- existir en credit_card.id antes de poder ser creado en transaction.credit_card_id.
SELECT *
FROM credit_card
WHERE id = 'CcU-9999'
;

-- Añadir el valor 'CcU-9999' en el campo id de la tabla credit_card.
INSERT INTO credit_card(id) 
VALUES ('CcU-9999')
;

-- Introducion de los nuevos datos.
...
-- NIVEL 1
-- EJERCICIO 4
-- De recursos humanos, se le pide que retire la columna "pan" de la tabla de crédito. 
-- Recuerda mostrar el cambio hecho.

-- comprobacion de las columnas de la tabla credit_card
SHOW COLUMNS
FROM credit_card
;
...
-- Comprobacion del registro a eliminar.
SELECT * 
FROM transaction
WHERE id = '02C6201E-D90A-1859-B4EE-88D2986D3B02'
;

-- Eliminacion del registro con ID 02C6201E-D90A-1859-B4EE-88D2986D3B02 de la base de datos.
DELETE
FROM transaction
WHERE id = '02C6201E-D90A-1859-B4EE-88D2986D3B02'
;
...
-- NIVEL 2
-- EJERCICIO 2
-- crear una vista llamada VistaMarketing que contiene la siguiente información: Nombre de la empresa. 
-- Número de teléfono de contacto. País de residencia. Compra media realizada por cada empresa. 
-- Presenta la imagen creada, ordenando los datos de mayor a menor de la compra media. 

CREATE VIEW VistaMarketing AS
SELECT company.company_name
, company.phone
, company.country
, AVG(transaction.amount) AS amount_average
FROM company
JOIN transaction 
...
-- Presentacion vista VistaMarketing.
SELECT *
FROM VistaMarketing
;
...
-- NIVEL 3
-- EJERCICIO 1
-- La próxima semana tendrá una nueva reunión con los gerentes de marketing. 
-- Un miembro del equipo hizo cambios en la base de datos, pero no recuerda cómo los hizo. 
-- Pídele que le ayude a dejar ejecutada los comandos para obtener el siguiente diagrama: 
...
-- Comprobacion de la tabla company
SHOW COLUMNS
FROM company
;

-- Eliminación de la columna website en la company.
ALTER TABLE company
DROP COLUMN website
;
...
-- modificacion de los datatype y creacion de la columna fecha_actual.
ALTER TABLE credit_card
...
-- Comprobacion de la tabla credit_card despues de la actualizacion.
SHOW COLUMNS
FROM credit_card
;
...
-- Comprobacion de las tablas 
SHOW TABLES
FROM transactions
;

-- modificacion de la tabla user a data_user.
ALTER TABLE user
RENAME TO data_user
;

-- Comprobacion de las tablas despues de la actualizacion. 
SHOW TABLES
FROM transactions
;

-- Comprobacion de los datos de la tabla data_user.
SHOW COLUMNS
FROM data_user
;

-- Modificacion del nombre de la columna 'email' a 'personal_email'
ALTER TABLE data_user
RENAME COLUMN email  TO personal_email
;

-- Comprobacion de los datos de la tabla data_user despues de la actualizacion. 
SHOW COLUMNS
FROM data_user
;

-- Comprobacion del esquema de la base de datos transactions. 
/*La relacion entre las tablas data_user y transaction no se corresponde 
con el esquema objetivo. 
Se elimina la relacion existente y se crea otra acorde al esquema objetivo. */

-- Eliminacion de la relacion data_user transaction
...
-- comprobacion del dato que genera el error referencial
-- en la creacion de la relacion.
...
-- creacion de un usuario en data_user con id = 9999
...
-- Creacion de la relacion entre las tablas data_user y transaction. 
ALTER TABLE transaction 
ADD foreign key (user_id) REFERENCES data_user(id)
;

-- Comprobacion del esquema de la base de datos transactions con el esquema objetivo.
-- Se comprueba que se corresponden.
...
-- Creacion de la vista InformeTecnico
CREATE VIEW InformeTecnico AS
SELECT transaction.id
, data_user.name AS name
, data_user.surname AS surname
, credit_card.IBAN AS account
...
-- Presentacion vista InformeTecnico. 
SELECT *
FROM InformeTecnico
;
